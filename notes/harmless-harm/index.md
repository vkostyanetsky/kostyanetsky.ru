Разбирали на днях с коллегой проблему. Ничего особенно серьёзного, очередный момент "какого черта этот запрос ведет себя так странно?".
...
Несмотря на кажущуюся аккуратность, такой трюк ведёт к проблемам с производительностью, если таблица, которую читает запрос — большая.
...
Решение тут простое — вставлять TRUE/FALSE в тело запроса как константу, не используя параметр. Либо использовать оператор TOP, так текст запроса будет проще:
